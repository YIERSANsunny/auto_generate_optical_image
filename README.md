...
其中 I路使用 `psi=0`，Q路使用GUI里的 `Q路RF相位`，默认 `psi=90 deg`。
...
鼠标悬停到可见边带时，提示框会显示视图、边带阶数、频偏、真实相位、线性幅度、归一化功率 dB、复数实部/虚部。

CSV 字段包括：

```text
view, order, freq_offset_ghz, magnitude, magnitude_db, phase_deg, real, imag
```
